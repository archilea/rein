...
Images are signed by canonical digest once per release, so every tag (`v0.1.0`, `0.1`, `latest`) that resolves to the same digest passes the same verification. See the [Verifying the image](./README.md#verifying-the-image) section of the README for the `cosign verify` command.
